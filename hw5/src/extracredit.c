...
map_node_t * getnotrecent(hashmap_t *self);
void removefromdl(hashmap_t *self,map_node_t *mapnode);
void maketorecent(hashmap_t *self,map_node_t * mapnode);
...
hashmap_t *create_map(uint32_t capacity, hash_func_f hash_function, destructor_f destroy_function) {
    if(capacity<=0 || hash_function==NULL || destroy_function==NULL){
      // if(capacity<0 || hash_function==NULL ){// destory ?????????????????????????????????
        errno=EINVAL;
        return NULL;
    }
   ...
   if( pthread_mutex_init(&(newMap->fields_lock),NULL) !=0){
    ...
    newMap->dl=calloc(1,sizeof(doublelinklist));//free later
...
bool put(hashmap_t *self, map_key_t key, map_val_t val, bool force) {
    ...
    map_node_t *eachnodestart;
    for (int index=0;index<self->capacity;index++){
        eachnodestart=(self->nodes)+index;
        if(keyequal( (eachnodestart->key),MAP_KEY(NULL, 0) )==false && eachnodestart->tombstone==false){
            if((ntime-(eachnodestart->ctime))>TTL){
                self->size--;
                (self->destroy_function)(eachnodestart->key,eachnodestart->val);
                eachnodestart->tombstone=true;
            }
        }
    ...
    if(self->capacity==self->size){//force have to be consider   //full hashmap
                              ...
                              if(force==false){
                               ...
                               if(loopmark==self->capacity){
                                ...
                                (self->destroy_function)(tempmapnode->key,tempmapnode->val);
                                 ...
                                 tempmapnode->val =val;
                                 tempmapnode->tombstone=false;
                                 tempmapnode->ctime=time(NULL);

                              //  self->size=(self->size)+1;
                                maketorecent(self,tempmapnode);
                                pthread_mutex_unlock(&(self->write_lock));
                                return true;
           ...
           while( loopmark!=self->capacity){//index 0???
                  ...
                  /*   if(temp ->tombstone ==true){
                     ...
                     if(keyequal(temp ->key,key ) && temp ->tombstone ==false){   //already have key
                         (self->destroy_function)(   temp ->key,   temp ->val);
                        break;
    ...
    if( keyequal(temp ->key,MAP_KEY(NULL, 0) ) || loopmark==self->capacity  ){//not key before,find tombstone
...
/*
 pthread_mutex_lock(&(self->fields_lock));
    ...
    map_node_t *eachnodestart;
    for (int index=0;index<self->capacity; index++){
        eachnodestart=(self->nodes);
        if(keyequal( (eachnodestart->key),MAP_KEY(NULL, 0) )==false){
            if((ntime-(eachnodestart->ctime))>TTL){
                self->size--;
                eachnodestart->tombstone=true;
            }
                ...
                if(returnvalue->tombstone==true){
                        ...
                        return MAP_VAL(NULL, 0);
                         ...
                         time_t ntime=time(NULL);
                                 ...
                                 if(((ntime-(returnvalue->ctime))>TTL)){
                                    pthread_mutex_lock(&(self->fields_lock));
...
void maketorecent(hashmap_t *self,map_node_t * mapnode){
    if((self->dl)->front==NULL && (self->dl)->rear==NULL){
       (self->dl)->front=mapnode;
       (self->dl)->rear=mapnode;
       mapnode->prev=NULL;
       mapnode->next=NULL;
    }else if((self->dl)->front!=NULL || (self->dl)->rear!= NULL){

               if(mapnode->prev==NULL && mapnode->next==NULL){
                   ((self->dl)->front)->prev=mapnode;
                   mapnode->prev=NULL;
                   mapnode->next=(self->dl)->front;
                   (self->dl)->front=mapnode;
               }else {
                   removefromdl(self,mapnode);///????????????/remove first
                   ((self->dl)->front)->prev=mapnode;
                   mapnode->next=(self->dl)->front;
                   mapnode->prev=NULL;
                   (self->dl)->front=mapnode;
...
map_node_t * getnotrecent(hashmap_t *self){//rear is not recent
    map_node_t *recentnode = (self->dl)->rear;
    return recentnode;
...
void removefromdl(hashmap_t *self,map_node_t *mapnode){
    ...
    }else if(mapnode->prev==NULL && mapnode->next == NULL){
        (self->dl)->front=NULL;
        (self->dl)->rear=NULL;
    }else if(mapnode->prev==NULL){
        (mapnode->next)->prev=NULL;
        (self->dl)->front=mapnode->next;
    }else if(mapnode->next==NULL){
        (mapnode->prev)->next=NULL;
        (self->dl)->rear=mapnode->prev;
    }
